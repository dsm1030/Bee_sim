classdef Quadrotorb < RigidBodyManipulator
  
  methods
    
    function obj = Quadrotorb(sensor,floating_base_type)
      ...
      obj = obj@RigidBodyManipulator(getFullPathFromRelativePath('nBEE.urdf'),options);
      warning(w);
    ...
    function u0 = nominalThrust(obj)
      ...
      u0 = Point(getInputFrame(obj),getMass(obj)*norm(getGravity(obj))*[1;0;0;0]);
%       u0 = Point(getInputFrame(obj),getMass(obj)*norm(getGravity(obj))*ones(4,1)/4);
    ...
    function obj = addPillar(obj, w, xy)
      % Adds a single pillar with specified height,
      % and xy location
      height = 8;
      width = w*ones(1,2);
      pillar = RigidBodyBox([width height],[xy;height/2],zeros(3,1));
      pillar.c = [90,90,90]/255;
      obj = addGeometryToBody(obj,'world',pillar);
      obj = compile(obj);
    end
